...
export default function WhyPorul() {
  return (
    <section className="py-24 px-8 bg-white relative overflow-hidden group/why-section">
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        <div className="text-center mb-16 space-y-8">
            <motion.div 
               ...
               className="inline-flex items-center gap-4 px-6 py-2 rounded-full glass-lvl-1 border border-black/10 text-[10px] font-black tracking-[0.4em] text-accent-violet mb-4 uppercase shadow-sm bg-white"
            >
            ...
            <motion.p 
               initial={{ opacity: 0, y: 15 }}
               whileInView={{ opacity: 1, y: 0 }}
               className="text-xl font-light text-black/40 leading-relaxed max-w-xl mx-auto font-inter italic"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
           {axioms.map((axiom, index) => (
             <motion.div
               ...
               className={`group/${axiom.id} relative p-10 rounded-[48px] border border-black/10 hover:border-black/20 transition-all duration-1000 overflow-hidden shadow-[0_32px_80px_-20px_rgba(0,0,0,0.06)] hover:shadow-2xl hover:scale-[1.02] bg-white group/pod`}
             >
                ...
                <div className="space-y-10 relative z-10">
                   <div className="flex justify-between items-start">
                      <div className={`w-14 h-14 rounded-2xl glass-lvl-1 flex items-center justify-center ${axiom.accent} bg-white shadow-sm group-hover/pod:bg-black group-hover/pod:text-white transition-all`}>
                         {axiom.icon}
                      </div>
                ...
                <div className="absolute -bottom-10 -right-10 text-[100px] font-black text-black/[0.015] uppercase tracking-tighter select-none pointer-events-none group-hover/pod:text-black/[0.03] transition-colors rotate-12">{axiom.id.split('-')[1]}</div>
             </motion.div>
           ))}
        </div>
      </div>
