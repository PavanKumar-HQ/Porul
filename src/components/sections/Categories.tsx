...
export default function Categories() {
  return (
    <section className="py-40 px-8 bg-[#FDFDFD] relative overflow-hidden group/cat-section">
      {/* Dynamic Grid Background (Increased Contrast) */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808015_1px,transparent_1px),linear-gradient(to_bottom,#80808015_1px,transparent_1px)] bg-[size:32px_32px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)]" />
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-12 text-black">
          <div className="max-w-xl">
            ...
            <motion.h2 
               ...
               className="text-5xl md:text-7xl font-bold font-outfit tracking-tighter leading-[0.9] mb-8"
            >
              Curated <br />
              <span className="text-black/15 italic text-black/20">Museum</span>
            </motion.h2>
          </div>
          <motion.p 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 md:auto-rows-[350px]">
          {categories.map((category, index) => (
            <motion.div
              ...
              className={`${category.span} group relative rounded-[40px] overflow-hidden border border-black/10 shadow-sm hover:shadow-2xl transition-all duration-1000 ${category.color}`}
            >
              ...
              <div className="p-10 relative z-20 flex flex-col h-full justify-between">
                 <div className="flex justify-between items-start">
                    <div className="w-14 h-14 rounded-2xl glass-lvl-3 flex items-center justify-center text-black/60 group-hover:bg-black group-hover:text-white transition-all duration-700 shadow-sm bg-white">
                       {category.icon}
                    </div>
                    <Link 
                      href={category.href}
                 ...
                 <div className="space-y-6">
                    <div>
                       ...
                       <p className="text-sm font-light text-black/50 leading-relaxed max-w-[280px]">{category.description}</p>
                    </div>
                    <Link 
                      href={category.href}
              ...
              <div className="absolute inset-x-8 bottom-8 top-32 overflow-hidden rounded-[32px] pointer-events-none group-hover:scale-[1.02] transition-transform duration-1000">
                 <img 
                    src={category.image} 
                    alt={category.name} 
                    className="w-full h-full object-cover opacity-30 grayscale group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-1000 blur-[30px] group-hover:blur-0" 
                 />
              </div>
            </motion.div>
          ))}
